...
def run_espresso(pla_single, final_name, outdir='Benchmarks_espresso'):
    if not os.path.exists(outdir):
        os.system(f'mkdir {outdir}')
    final_path = '%s/%s' % (outdir, final_name)
    print(f'espresso --fast {pla_single} > {final_path}')
    os.system(f'espresso --fast {pla_single} > {final_path}')
    return f'{final_path}'
