...
for pla_path in train_file_list:
    ...
    for idx in range(Y.shape[1]):
        ...
        time_table.append([pla_path.replace('.pla', f'({idx}).pla'), "write_single_out_pla", num_feats,
                           time_f - time_i])

        time_i = time.time()
        aig = gen_aig(pla_single)
        time_f = time.time()
        total_time += time_f - time_i
        ...
        time_i = time.time()
        ands, lev, acc_abc = run_aig(aig, pla_single, outdir="tmp")
        time_f = time.time()
        total_time += time_f - time_i
        time_table.append([pla_path.replace('.pla', f'({idx}).pla'), "run_aig", num_feats, time_f - time_i])

        print(",".join([str(x) for x in [pla_path, nr_inputs, nr_outputs, idx, acc_abc, ands, lev]]), file=f_out)
